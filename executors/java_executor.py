...
def find_class(source):
    ...
    if class_name is None:
        raise CompileError('El main debe estar dentro de una public class. ¡Como si fuera Java normal!"\n')
    package = repackage.search(source)
    if package:
        raise CompileError('Por favor, borra la linea del package\n')
    return class_name

def remove_package(source):
    package = repackage.search(source)
    if package:
        out = repackage.sub("",source)
        return out
    return source
    ...
    def parse_feedback_from_stderr(self, stderr, process):
        ...
        if b'Error: Main method not found in class' in stderr:
            ...
            if not match:
                ...
                exception = ("Has intentat hackejar-me. Et sentencio a mort.\n")
            else:
                exception = match[0].group(1).split(':')[0]+" en línea "+linea
...
class JavacExecutor(JavaExecutor):
    def create_files(self, problem_id, source_code, *args, **kwargs):
        ...
        source_code = remove_package(source_code)
